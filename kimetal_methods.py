...
def eval_baseline(input_data, output_data):
    ...
    for round in data:
        rank = list()
        unknown_word = False
        remaining_words = data[round]["remaining words"]
        clue = data[round]["clue"]
        gold_guesses = data[round]["guesses"]
...
def main():
    eval_baseline("data/ZiS_clean_glove.json", "results/baseline_ranking_eval_new.tsv")
